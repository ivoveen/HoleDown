...
void main()
{
	...
	char fs[] =
		...
		//vignette shader created with help from Jaeden 
		"vec2 pixel = (vt.xy * 2) - vec2(1); \n"
		"float vig = pow(VIGNETTEBRIGHTNESS * (abs(pixel.x)-1) * ( abs(pixel.y) -1), 0.5); \n"
		"dmw *= vec4(1.0,1.0,1.0,1.0) * vig; \n"
	...
	Shader* shaderNormal = new Shader(
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (frameNr++ > 1)
		{
			...
			if (app->getShadersOn()) {
				...
				shaderNormal->Bind();
				shaderNormal->SetInputTexture(0, "c", renderTarget);
				shaderNormal->Bind();
				shaderNormal->SetInputTexture(0, "c", renderTarget);
				DrawQuad();
				shaderNormal->Unbind();
