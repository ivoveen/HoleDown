...
void Surface::Print( const char* s, int x1, int y1, uint c,int factor)
{
	if (!fontInitialized)
	{
	...
	for (int i = 0; i < (int)(strlen( s )); i++, t += 6 * factor)
	{
		int pos = 0;
		...
		for (int v = 0; v < 5 * factor; v+= factor, u++, a += width * factor)
			for (int h = 0; h < 5 * factor; h+= factor) {
				if (*u++ == 'o') {
...
void Surface::CopyTo( int copyFromY, Surface* d, int x, int y )
{
	uint* dst = d->pixels;
	uint* src = pixels;
	if ((src) && (dst))
